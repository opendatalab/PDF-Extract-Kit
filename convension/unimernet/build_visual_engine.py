...
if __name__ == '__main__':
    # processor = NougatProcessor.from_pretrained(args.model_path)
    # raw_image = Image.new('RGB', [10, 10])  # dummy image
    # image = processor(raw_image, return_tensors="pt")['pixel_values'].to(args.device, torch.float16)
    import sys
    ...
    model = VisionEncoderDecoderModel.from_pretrained("weights/unimernet_clean",torch_dtype=torch.float16)
    swin_encoder = model.get_encoder().to(device)
    wrapper = SwinEncoderWrapper(swin_encoder)
    image = torch.randn(max_batch_size, 3, 192, 672).to(device, torch.float16)
    export_onnx(wrapper, image, f'{output_dir}/onnx')
    build_trt_engine(model_type,
        [image.shape[1], image.shape[2], image.shape[3]],  # [3, H, W]
        f'{output_dir}/onnx',
        output_dir,
        max_batch_size)
