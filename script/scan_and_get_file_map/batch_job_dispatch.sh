...
FILEPATH=scihub_collection/sci_index_files.finished.filelist #scihub_collection/sci_index_files.filelist
JOBSCRIPT=script/scan_and_get_file_map/scan_and_track_id_for_each_file.py
...
do  
    ...
    sbatch  -p ai4earth -N1 -c20 --gres=gpu:0 script/scan_and_get_file_map/job_dispatch.sh $FILEPATH $START $PARTION_INTERVAL $PARTION_TOTAL $JOBSCRIPT
    sleep 1
done
