...
FILEPATH=scihub_collection/sci_hub.finished.filelist
JOBSCRIPT=script/scan_and_judge_det_ocr_status/scan_and_judge_ready_for_ocr.py
...
do  
    ...
    sbatch  -p ai4earth -N1 -c20 --gres=gpu:0 script/scan_and_judge_det_ocr_status/job_dispatch.sh $FILEPATH $START $PARTION_INTERVAL $PARTION_TOTAL $JOBSCRIPT
    sleep 1
done
